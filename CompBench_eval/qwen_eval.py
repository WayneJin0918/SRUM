...
# --- Model and Processing Dependencies ---
import torch
from transformers import Qwen2_5_VLForConditionalGeneration, AutoProcessor
from qwen_vl_utils import process_vision_info # A utility for handling vision data
...
# =====================================================================================
# Helper Function to Load Prompts from JSONL File
# =====================================================================================
def load_prompts_from_jsonl(file_path: str) -> Dict[str, str]:
    """Loads data from a JSONL file and creates a mapping dictionary from image_file to original_prompt."""
    prompts_map = {}
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    # Ensure both required keys are in the JSON object
...
def parse_args():
    ...
    # [MODIFIED] Added --start and --end arguments for slicing the dataset
...
def extract_score(evaluation_text: str) -> int:
    """Extracts the score from the model's text output."""
    try:
        # First, try to parse the score from a clean JSON object
        cleaned_text = evaluation_text.strip().replace('```json', '').replace('```', '').strip()
        # Use regex to find a JSON-like structure containing "score"
        ...
        # Log if JSON parsing fails, but proceed to the next method
        logging.warning(f"Failed to parse score from JSON. Text: '{evaluation_text}'. Error: {e}")

    # As a fallback, use regex to find the score directly
    pattern = r'"score":\s*"?(\d+)"?'
    match = re.search(pattern, evaluation_text)
    if match:
        return int(match.group(1))

    # If both methods fail, return an error code
    logging.warning(f"Could not extract score from text: '{evaluation_text}'")
    return -1

def save_to_csv(results: List[Dict], output_path: str):
    """Saves the list of results to a CSV file and appends summary statistics."""
    if not results:
...
def process_batch(batch_data: List[Dict], model, processor) -> List[Dict]:
    ...
    # Prepare the input messages for each item in the batch
    ...
    try:
        # Apply chat template and process vision/text inputs
        ...
        # Generate model output without gradient calculation for efficiency
        with torch.no_grad():
        ...
        # If an error occurs during batch processing, mark all items in the batch as failed
        logging.exception("Failed to process a batch.")
        for item in batch_data:
            item['score'] = -1
            item['full_evaluation_text'] = f"BATCH_PROCESSING_ERROR: {e}"
...
def main():
    ...
    # Determine the path for the prompts file (handles `_no_think` and `_think` variations)
    ...
    # Load prompts; exit if the file is not found
    ...
    # Prepare the list of tasks to be evaluated
    logging.info("Preparing evaluation tasks...")
    tasks_to_process = []
    try:
        # Check for a 'samples' subdirectory, otherwise use the main directory
        image_path_total = os.path.join(current_image_path, "samples")
        if not os.path.exists(image_path_total):
            image_path_total = current_image_path
        
        # Filter for allowed image file extensions
        ...
        if not image_files:
            logging.error(f"No image files found in {image_path_total}")
            return
        logging.info(f"Found {len(image_files)} image files to process.")
        
        # Sort files numerically based on their filenames (e.g., 1.png, 2.png, ...)
        image_files.sort(key=lambda x: int(os.path.splitext(x)[0]))
    except Exception:
        logging.exception(f"Error reading or sorting image files from {current_image_path}.")
        return

    # Create a task dictionary for each valid image file
    for image_file in image_files:
        try:
            # Match image file with its corresponding prompt
            ...
            # Construct the detailed prompt for the vision-language model
            question_text = (
            ...
            logging.warning(f"Skipping file with non-numeric name: {image_file}")
            continue
    
    # [MODIFIED] Apply start, end, and step slicing to the task list
    final_tasks = tasks_to_process[args.start:args.end:args.step]
    if not final_tasks:
        logging.warning("No tasks to process after applying start/end/step filters.")
        return
    logging.info(f"Generated {len(final_tasks)} tasks to process for this chunk.")

    # Split the final tasks into batches
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_workers) as executor:
        ...
        progress_bar = tqdm(
            ...
            total=len(batches), 
            desc=f"Processing chunk {args.start}-{args.end}"
        )
    ...
    # Save all collected results to the output CSV file
