...
# 从 0.1 到 1.0 循环，步长为 0.1
for lambda_val in $(seq 0.1 0.1 1.0)
do
  # 根据当前的 lambda 值设置检查点目录名
  checkpoint_path="results/checkpoint_reg_2e5_${lambda_val}"
  echo "Staring training with lambda_constraint: ${lambda_val}"
  ...
  torchrun \
    ...
    --lambda_constraint ${lambda_val}

done

echo "All training runs have been completed."